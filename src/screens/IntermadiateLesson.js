...
const IntermadiateLesson = () => {
  const [IntermadiateData, setIntermadiateData] = useState([]);
  ...
  const fetchData = async () => {
    try {
      const IntermadiateCollection = collection(db, "intermadiate");
      const IntermadiateQuery = query(IntermadiateCollection, orderBy("no", "asc"));
      const querySnapshot = await getDocs(IntermadiateQuery);
      const IntermadiateLessons = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        url: doc.data().url,
        name: doc.data().name,
        desc: doc.data().desc,
      ...
      setIntermadiateData(IntermadiateLessons);
    } catch (error) {
...
const styles = StyleSheet.create({
  backgroundImage: {
    ...
    export default IntermadiateLesson;
